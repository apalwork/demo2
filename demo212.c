...
BYTE enter_play_mode(BYTE seg)
{
	...
	*/
	if(seg==0) try_play=API_PSTARTH(P0);
	else if(seg==1) try_play=API_PSTARTH(P1);
	else try_play=api_play_start_with_state(R3_STARTPAGE, new_end_page, 0x17f, R3_ULAW, API_DAOSR_HIGH, &the_state);
	if(try_play)
		sys_state=SYS_PLAY;
	playing_seg=seg;
...
void enter_rec_mode(void)
{
	...
	THRLD=0; // slower clock
	wait_beep(REC_WAIT_TIME);
	THRLD=TMR_RLD;
	if(key_code)
	{
		api_rec_stop(0);
	...
	if(!api_rec_start(0xff, API_REC_FG_DEFAULT,R3_ULAW,
		R3_STARTPAGE,R3_ENDPAGE,callbackchk))
	{
		api_rec_stop(0);
		return;		
	}
...
BYTE sys_rec(void)
{
	...
	if(rec_stage==4) // compare
	{
		if(api_rec_job_no_write(&the_state, &newpwr)!=0)		
		{
			...
			/*
			the_index = (rindex-1)&3;
			the_pred=oldpredict[the_index];			
			the_index=oldpwr[the_index];
			*/
